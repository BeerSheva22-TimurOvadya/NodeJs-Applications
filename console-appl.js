...
let promptAsync = new PromptAsync();
const handler = new PromptHandler(promptAsync);
...
async function asyncTyping() {
    ...
    if (num1) {
        ...
        appleOrange = await promptAsync.readPredicate(
            'Enter either apple or orange',
            'neither apple nor orange',
            (answer) => answer == 'apple' || answer == 'orange',
        );
